...
public class MainActivity extends ViewPagerActivity {
  ...
  private BluetoothAdapter.LeScanCallback mLeScanCallback = new BluetoothAdapter.LeScanCallback() {
    ...
    public void onLeScan(final BluetoothDevice device, final int rssi, byte[] scanRecord) {
    	//20140108
    	ParcelUuid uuids[];   //same as  ParcelUuid[] uuids== device.getUuids();
    	uuids= device.getUuids();
    	
    	if(uuids==null)
    		System.out.println(device.getAddress()+"   RSSI:"+rssi); 
    	else
    	  {for(int i=0;i<uuids.length;i++)
    		System.out.println("UUID:"+uuids[i].toString()+"  "+device.getAddress()+"   RSSI:"+rssi); 
    		System.out.println("uuids_length:"+uuids.length);
    		}
    	//20140108
